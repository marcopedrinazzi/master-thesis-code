...
contract CertificationModel{
    ...
    uint public constant SIZE = 3; //size of the evidence collection model
    ...
    constructor(string memory _non_functional_property, string memory _target_of_certification, address _apiConsumerAddr, address _preCoordinatorAddr, bytes32 _jobIdHeartBleed, bytes32 _jobIdObservatory, bytes32 _jobIdSslyze){
...
contract APIConsumer is ChainlinkClient, ConfirmedOwner {
    ...
    uint256 constant private FEE = 3 * (0.1 * 10**18); // 5 (the number of oracles (in the well-known list) in the network) * 0.1 LINK

...
contract PreCoordinator is ChainlinkClient, Ownable, ChainlinkRequestInterface, LinkTokenReceiver {
  ...
  {
    //init oracles array
    address[] memory _oracles = new address[](3);
    for(uint256 i = 0; i<3; i++){ //5 is the number of oracles that i assume in the system
      uint256 index_random = _randomIndexes[i];
      _oracles[i]=fixed_oracle_list[index_random];
    }
    require(_jobIds[0]==_jobIds[1] && _jobIds[1]==_jobIds[2]); //job ids must be all equal for our purpose

...
contract CertificationModel{
    ...
    function test1() private {
        ...
        api.requestCompletedData(model.preCoordinatorAddr,model.jobId); //it executes the test
    }
...
contract APIConsumer is ChainlinkClient, ConfirmedOwner {
    ...
    uint256 constant private FEE = 4 * (0.1 * 10**18); // 4 (the number of oracles (in the well-known list) in the network) * 0.1 LINK
    ...
    */
    function requestCompletedData(address _oracleAddr, bytes32 jobId) public returns (bytes32 requestId) {

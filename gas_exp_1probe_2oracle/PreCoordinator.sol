...
contract PreCoordinator is ChainlinkClient, Ownable, ChainlinkRequestInterface, LinkTokenReceiver {
  ...
  address[] public fixed_oracle_list =  [0xCC79157eb46F5624204f47AB42b3906cAA40eaB7, 0xCC79157eb46F5624204f47AB42b3906cAA40eaB7];
    ...
    external returns (bytes32 saId)
  {
    ...
    for(uint256 i = 0; i<2; i++){ //2 is the number of oracles that i assume in the system
      uint256 index_random = _randomIndexes[i];
      _oracles[i]=fixed_oracle_list[index_random];
    }
    require(_jobIds[0]==_jobIds[1]); //job ids must be all equal for our purpose
